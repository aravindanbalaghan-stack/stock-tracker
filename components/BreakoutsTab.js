    ...
    <div>
      ...
      {rows.length === 0 ? (
        <div className="rounded-lg border py-12 text-center text-sm" style={{ borderColor: "var(--border)", background: "var(--surface)", color: "var(--text-muted)" }}>
          No stocks currently clear all three filters.
        </div>
      ) : (
      ...
      <p className="mt-3 text-xs" style={{ color: "var(--text-faint)" }}>
        Delivery % above {data.criteria?.deliveryPctMin ?? 70}%, price up more than{" "}
        {data.criteria?.changePctMin ?? 1}% vs previous close, and volume at least{" "}
        {data.criteria?.volumeRatioMin ?? 2}× the 30-day average — a same-day surge screen, not a
        confirmed signal.
      </p>
    </div>
  );
