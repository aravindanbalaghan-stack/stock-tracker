...
export function buildRecentPeriodHistory(symbol, days, periodTradingDays = 1, periods = HISTORY_PERIODS) {
  ...
  return chunks.map((chunkDays) => {
    ...
    return {
      ...
      deliveryPct,
      volume: volume || null,
      // Delivered SHARES, not just the ratio. Delivery % is
      // delivered ÷ traded, so when speculative volume dries up the
      // percentage climbs even as fewer shares are actually being taken
      // — the ratio rises purely because its denominator shrank. Showing
      // the absolute quantity alongside makes that visible instead of
      // letting a collapsing sector read as strengthening accumulation.
      deliveryQty: deliveryQty || null,
      // Price move across the bucket, so the trend strip says whether the
      // delivery was happening into strength or weakness.
      changePercent:
