// Data + signal logic for the NIFTY BANK Trading tab. Separate from
// lib/wma.js because that file's fetchDailySeries hardcodes the ".NS"
// suffix used for individual stocks — indices like ^NSEBANK use their raw
// Yahoo symbol instead (same distinction lib/indices.js already makes).

import { INDEX_CONSTITUENTS } from "@/lib/indexConstituents";

const UA =
  "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0 Safari/537.36";

export const NIFTY_BANK_SYMBOL = "^NSEBANK";

// The index itself has no real trade volume (see hasRealVolumeData's
// comment below) — but its 12 constituents do. Reusing the same
// hand-maintained list the Top Indices tab already has for ^NSEBANK
// means there's one source of truth instead of two.
export const NIFTY_BANK_CONSTITUENTS = INDEX_CONSTITUENTS["^NSEBANK"] ?? [];

// Yahoo's own hard limit, not a choice made here: intraday data (any
// interval under 1 day) is only retained for ~60 days, regardless of
// candle size. There is no way to get a genuine 3-month intraday
// backtest from this data source — 60 days (roughly the last 2 months of
// trading) is the practical ceiling. See NiftyBankTab.js for how this is
// surfaced to the person using the app.
export const MAX_INTRADAY_LOOKBACK_DAYS = 58; // slightly under 60 as a safety margin against Yahoo's own edge

const IST_TZ = "Asia/Kolkata";

function istDateKey(epochSeconds) {
  // en-CA gives YYYY-MM-DD directly, which is what every other date key
  // in this codebase (bhavcopy dates, etc.) already uses.
  return new Date(epochSeconds * 1000).toLocaleDateString("en-CA", { timeZone: IST_TZ });
}

async function fetchChart(symbol, interval, range) {
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(
    symbol
  )}?interval=${interval}&range=${range}`;
  try {
    const res = await fetch(url, {
      headers: { "User-Agent": UA, Accept: "application/json" },
      cache: "no-store",
    });
    if (!res.ok) return null;
    const data = await res.json();
    const result = data?.chart?.result?.[0];
    if (!result) return null;

    const timestamps = result.timestamp || [];
    const quote = result.indicators?.quote?.[0] || {};
    const bars = timestamps
      .map((t, i) => ({
        t,
        o: quote.open?.[i] ?? null,
        h: quote.high?.[i] ?? null,
        l: quote.low?.[i] ?? null,
        c: quote.close?.[i] ?? null,
        v: quote.volume?.[i] ?? null,
      }))
      .filter((b) => b.h != null && b.l != null && b.c != null);
    return bars;
  } catch {
    return null;
  }
}

// Daily OHLCV for the index — used for the 21 EMA (close-based, so the
// index's own always-0 volume doesn't matter here). range=2y gives enough
// history for both a stable EMA and 30+ weeks of completed weekly buckets
// even after dropping the current in-progress week.
export async function fetchDailyBars(range = "2y") {
  return fetchChart(NIFTY_BANK_SYMBOL, "1d", range);
}

// Intraday OHLC for the index (price levels only — volume comes from
// fetchConstituentIntradayVolumeByBucket instead, see below). `range`
// must respect Yahoo's ~60-day retention window for sub-daily intervals —
// see MAX_INTRADAY_LOOKBACK_DAYS.
export async function fetchIntradayBars(interval = "5m", range = "60d") {
  return fetchChart(NIFTY_BANK_SYMBOL, interval, range);
}

const CONSTITUENT_FETCH_CONCURRENCY = 6;

// Sums each constituent stock's intraday volume into 5-minute buckets
// (or whatever `interval` implies) and adds the buckets together — this
// is the NIFTY BANK "volume" actually used for signal detection, since
// the index itself reports none. Bucketing by a fixed epoch-aligned
// window (rather than trusting each stock's bar timestamps to line up
// exactly) is what makes summing across 12 independently-fetched series
// safe: NSE's 9:15 IST open lands exactly on a 5-minute UTC boundary, so
// flooring every bar's timestamp to the nearest bucket consistently
// groups the "same" 5 minutes across stocks even if Yahoo's per-symbol
// bar generation has minor jitter.
export async function fetchConstituentIntradayVolumeByBucket(interval = "5m", range = "60d") {
  const bucketSeconds = interval === "5m" ? 300 : interval === "15m" ? 900 : interval === "10m" ? 600 : 300;
  const totals = new Map(); // bucketEpoch -> summed volume
  const contributingSymbols = new Set(); // which constituents actually returned data, for diagnostics

  for (let i = 0; i < NIFTY_BANK_CONSTITUENTS.length; i += CONSTITUENT_FETCH_CONCURRENCY) {
    const batch = NIFTY_BANK_CONSTITUENTS.slice(i, i + CONSTITUENT_FETCH_CONCURRENCY);
    const results = await Promise.all(
      batch.map((symbol) => fetchChart(`${symbol}.NS`, interval, range).catch(() => null))
    );
    results.forEach((bars, idx) => {
      if (!bars || bars.length === 0) return;
      contributingSymbols.add(batch[idx]);
      for (const bar of bars) {
        if (bar.v == null) continue;
        const bucket = Math.floor(bar.t / bucketSeconds) * bucketSeconds;
        totals.set(bucket, (totals.get(bucket) ?? 0) + bar.v);
      }
    });
  }

  return { totals, contributingSymbols: [...contributingSymbols], bucketSeconds };
}

// Same idea as above but for daily bars, keyed by IST calendar date
// instead of an epoch bucket — daily bars are one-per-trading-day so a
// date-string key is simpler and doesn't depend on intraday alignment.
export async function fetchConstituentDailyVolumeByDate(range = "2y") {
  const totals = new Map(); // "YYYY-MM-DD" -> summed volume
  const contributingSymbols = new Set();

  for (let i = 0; i < NIFTY_BANK_CONSTITUENTS.length; i += CONSTITUENT_FETCH_CONCURRENCY) {
    const batch = NIFTY_BANK_CONSTITUENTS.slice(i, i + CONSTITUENT_FETCH_CONCURRENCY);
    const results = await Promise.all(
      batch.map((symbol) => fetchChart(`${symbol}.NS`, "1d", range).catch(() => null))
    );
    results.forEach((bars, idx) => {
      if (!bars || bars.length === 0) return;
      contributingSymbols.add(batch[idx]);
      for (const bar of bars) {
        if (bar.v == null) continue;
        const key = istDateKey(bar.t);
        totals.set(key, (totals.get(key) ?? 0) + bar.v);
      }
    });
  }

  return { totals, contributingSymbols: [...contributingSymbols] };
}

// Overlays aggregated constituent volume onto the index's own price bars,
// matching each bar to its 5-minute bucket. Bars with no matching bucket
// (a stock-only holiday quirk, or a bucket where every constituent
// happened to be missing data) keep v: null rather than silently
// becoming 0, so "no data" and "genuinely zero volume" stay distinguishable.
export function attachIntradayVolume(bars, volumeByBucket, bucketSeconds = 300) {
  return bars.map((bar) => {
    const bucket = Math.floor(bar.t / bucketSeconds) * bucketSeconds;
    const v = volumeByBucket.get(bucket);
    return { ...bar, v: v ?? null };
  });
}

// Same overlay for daily bars, keyed by IST date instead of a bucket.
export function attachDailyVolume(bars, volumeByDate) {
  return bars.map((bar) => {
    const key = istDateKey(bar.t);
    const v = volumeByDate.get(key);
    return { ...bar, v: v ?? null };
  });
}

// Groups intraday bars by IST trading day, sorted ascending both across
// days and within each day (Yahoo already returns bars in order, but this
// doesn't assume that).
export function groupByTradingDay(bars) {
  const byDay = new Map();
  for (const bar of bars) {
    const key = istDateKey(bar.t);
    if (!byDay.has(key)) byDay.set(key, []);
    byDay.get(key).push(bar);
  }
  for (const dayBars of byDay.values()) dayBars.sort((a, b) => a.t - b.t);
  return [...byDay.entries()].sort((a, b) => (a[0] < b[0] ? -1 : 1)); // [ [date, bars[]], ... ] oldest first
}

// Standard EMA, causal (each value only depends on itself and earlier
// values) — index i of the returned array is "as of" values[i], so using
// series[i-1] for a signal computed at bar i avoids lookahead bias.
export function computeEMA(values, period) {
  if (!values || values.length === 0) return [];
  const k = 2 / (period + 1);
  const out = [values[0]];
  for (let i = 1; i < values.length; i++) {
    out.push(values[i] * k + out[i - 1] * (1 - k));
  }
  return out;
}

// Resamples daily bars into one summed volume per ISO week (Mon-Sun),
// oldest first — same week-bucketing convention as lib/wma.js's
// toWeeklyCloses, just summing volume instead of taking the last close.
export function toWeeklyVolumes(dailyBars) {
  const weeks = new Map();
  for (const bar of dailyBars) {
    if (bar.v == null) continue;
    const d = new Date(bar.t * 1000);
    const monday = new Date(d);
    const day = (monday.getUTCDay() + 6) % 7;
    monday.setUTCDate(monday.getUTCDate() - day);
    const key = monday.toISOString().slice(0, 10);
    weeks.set(key, (weeks.get(key) ?? 0) + bar.v);
  }
  return [...weeks.entries()].sort((a, b) => (a[0] < b[0] ? -1 : 1)); // [ [weekStart, volume], ... ]
}

function average(arr) {
  return arr.length > 0 ? arr.reduce((a, b) => a + b, 0) / arr.length : null;
}

// The multiplier is 1 by default — "good volume" means simply above the
// rolling average, not some multiple of it. Still adjustable per-request
// (see the UI's volume multiplier input) for anyone who wants a stricter
// bar.
export const GOOD_VOLUME_MULTIPLIER = 1;

// NIFTY BANK is an index, not a traded instrument — only its constituent
// stocks and derivatives actually trade, so Yahoo's own quote page for
// ^NSEBANK shows "Volume: 0", and its intraday chart data reports 0 for
// every candle. The routes work around this by overlaying real volume
// aggregated from the 12 constituent stocks (see
// fetchConstituentIntradayVolumeByBucket/attachIntradayVolume above)
// before bars ever reach this function. hasRealVolumeData is the
// fallback check for the unlikely case that overlay comes back empty too
// (e.g. all 12 constituent fetches failed) — without it, a volume filter
// requiring "> some multiple of a 0 average" would silently fail every
// single day regardless of price action.
export function hasRealVolumeData(bars) {
  return (bars || []).some((b) => (b.v ?? 0) > 0);
}

// Combines every `groupSize` consecutive 5-minute bars into one larger
// candle (open of the first, close of the last, high/low across all,
// volume summed). Used to build genuine 10-minute candles from 5-minute
// ones rather than approximating a 10-minute high as "max of two 5-minute
// highs" — the aggregated candle has its own real volume, which the
// 10-minute breakout check needs. NSE's 375-minute session (9:15-15:30)
// isn't evenly divisible by 10 minutes, so the last candle of a day may
// come out shorter than the rest — that's expected, not a bug.
export function aggregateCandles(bars, groupSize) {
  const out = [];
  for (let i = 0; i < bars.length; i += groupSize) {
    const chunk = bars.slice(i, i + groupSize);
    if (chunk.length === 0) continue;
    out.push({
      t: chunk[0].t,
      endT: chunk[chunk.length - 1].t, // last member's own timestamp — see detectOpeningRangeBreakout
      o: chunk[0].o,
      h: Math.max(...chunk.map((b) => b.h)),
      l: Math.min(...chunk.map((b) => b.l)),
      c: chunk[chunk.length - 1].c,
      v: chunk.reduce((sum, b) => sum + (b.v ?? 0), 0),
    });
  }
  return out;
}

export const ROLLING_VOLUME_WINDOW = 30;

// Builds a continuous (spans day boundaries, same as a volume moving
// average on any charting platform), timeframe-aggregated candle series,
// plus a rolling trailing-N-candle volume average for every candle in it
// — this is the actual volume baseline used for "good volume": average
// of the last 30 candles of THAT candle's own timeframe (30 five-minute
// candles for the 5-minute check, 30 ten-minute candles for the 10-minute
// check), not a same-day or time-of-day baseline. Aggregation itself
// still respects day boundaries (a 10-minute candle never spans two
// different trading days — see aggregateCandles), but the rolling window
// is free to reach back into the previous day(s) once the current day
// doesn't have 30 candles yet, same as how any volume SMA behaves.
export function buildRollingVolumeSeries(groupedDays, candleMinutes, rollingWindow = ROLLING_VOLUME_WINDOW) {
  const groupSize = Math.max(1, Math.round(candleMinutes / 5));
  const continuous = [];
  for (const [, dayBars] of groupedDays) {
    continuous.push(...aggregateCandles(dayBars, groupSize));
  }
  const rollingAvgByTime = new Map();
  for (let i = 0; i < continuous.length; i++) {
    const windowStart = Math.max(0, i - rollingWindow);
    const vols = continuous
      .slice(windowStart, i)
      .map((b) => b.v)
      .filter((v) => v != null);
    rollingAvgByTime.set(continuous[i].t, average(vols));
  }
  return { continuous, rollingAvgByTime, groupSize };
}

function goodVolumeCheck(bar, { requireVolume, hasVolumeData, rollingAvgByTime, volumeMultiplier }) {
  if (!requireVolume || !hasVolumeData) return true;
  const avg = rollingAvgByTime?.get(bar.t);
  return avg != null && avg > 0 && (bar.v ?? 0) > avg * volumeMultiplier;
}

// Opening-range-breakout detection for one trading day's 5-minute bars.
// Two independent-timeframe, sequential-in-time checks:
//  1. "5-min breakout" — some 5-minute candle after the first CLOSES
//     above the first candle's high, on qualifying volume (vs. the
//     rolling 30-candle 5-minute volume average).
//  2. "10-min breakout" — some 10-minute candle (built by aggregating
//     this day's 5-minute bars, see aggregateCandles) AFTER the 5-min
//     breakout's own time CLOSES above the first 10-minute candle's
//     high, on qualifying volume (vs. the rolling 30-candle 10-minute
//     volume average) — confirming the 5-min move rather than an
//     unrelated later move.
// Both stages must fire for the day to count as a "setup". Breaks are
// judged on CLOSE price, not high/low wicks. `rollingAvg5ByTime`/
// `rollingAvg10ByTime` should come from buildRollingVolumeSeries() run
// once over the whole fetched dataset (not per-day) — see the routes
// that call this. Pass requireVolume: false to ignore volume entirely
// and flag every close-price break.
export function detectOpeningRangeBreakout(
  dayBars,
  {
    volumeMultiplier = GOOD_VOLUME_MULTIPLIER,
    hasVolumeData = true,
    requireVolume = true,
    rollingAvg5ByTime = null,
    rollingAvg10ByTime = null,
  } = {}
) {
  if (!dayBars || dayBars.length < 3) return null;

  const first5High = dayBars[0].h;
  const tenMinCandles = aggregateCandles(dayBars, 2);
  if (tenMinCandles.length < 1) return null;
  const first10High = tenMinCandles[0].h;

  let breakout5 = null;
  for (let i = 1; i < dayBars.length; i++) {
    const bar = dayBars[i];
    if (
      bar.c > first5High &&
      goodVolumeCheck(bar, { requireVolume, hasVolumeData, rollingAvgByTime: rollingAvg5ByTime, volumeMultiplier })
    ) {
      breakout5 = { time: bar.t, price: bar.c, volume: bar.v };
      break;
    }
  }

  let breakout10 = null;
  if (breakout5) {
    for (let k = 1; k < tenMinCandles.length; k++) {
      const bar = tenMinCandles[k];
      // A 10-min candle is only eligible to confirm once it extends to or
      // past the 5-min breakout's own time — using the candle's END
      // (not start) matters because a 10-min candle can legitimately
      // CONTAIN the 5-min breakout candle as its first half and still
      // extend meaningfully beyond it (a single strong candle breaking
      // both levels close together is a valid, even stronger, signal).
      if (bar.endT < breakout5.time) continue;
      if (
        bar.c > first10High &&
        goodVolumeCheck(bar, { requireVolume, hasVolumeData, rollingAvgByTime: rollingAvg10ByTime, volumeMultiplier })
      ) {
        breakout10 = { time: bar.t, price: bar.c, volume: bar.v };
        break;
      }
    }
  }

  return {
    first5High,
    first10High,
    dayVolume: dayBars.reduce((sum, b) => sum + (b.v ?? 0), 0),
    hasVolumeData,
    breakout5,
    breakout10,
    triggered: !!(breakout5 && breakout10),
  };
}

// "What happened next" for a triggered day — price action from just
// after the breakout's own time through the end of the available session
// data for that day. Works off `breakout.time` rather than an array
// index so it's agnostic to which timeframe (5-min bars vs. aggregated
// 10-min candles) the breakout was found on.
export function summarizeAfterBreakout(dayBars, breakout) {
  const afterIndex = dayBars.findIndex((b) => b.t > breakout.time);
  const after = afterIndex === -1 ? [] : dayBars.slice(afterIndex);
  const lastBar = dayBars[dayBars.length - 1];
  const highAfter = after.length > 0 ? Math.max(...after.map((b) => b.h)) : breakout.price;
  const lowAfter = after.length > 0 ? Math.min(...after.map((b) => b.l)) : breakout.price;
  const dayClose = lastBar.c;
  const pctFrom = (target) => (breakout.price ? ((target - breakout.price) / breakout.price) * 100 : null);

  return {
    dayClose,
    dayHighAfterBreakout: highAfter,
    dayLowAfterBreakout: lowAfter,
    closeVsBreakoutPct: pctFrom(dayClose) != null ? Math.round(pctFrom(dayClose) * 100) / 100 : null,
    maxGainPct: pctFrom(highAfter) != null ? Math.round(pctFrom(highAfter) * 100) / 100 : null,
    maxDrawdownPct: pctFrom(lowAfter) != null ? Math.round(pctFrom(lowAfter) * 100) / 100 : null,
  };
}

// Swing-high (pivot high) detection on a continuous 10-minute candle
// series: a candle at index i is a swing high if its high is strictly
// greater than the highs of the `lookback` candles before it AND the
// `lookforward` candles after it — the standard "fractal" pivot
// definition. A swing high is only confirmed once `lookforward` candles
// have printed after it (otherwise you'd need to see the future to know
// it was a local peak), so the most recent `lookforward` candles in the
// series can never produce a confirmed swing high yet.
export function findSwingHighs(tenMinContinuous, { lookback = 2, lookforward = 2 } = {}) {
  const swings = [];
  for (let i = lookback; i < tenMinContinuous.length - lookforward; i++) {
    const candle = tenMinContinuous[i];
    let isSwingHigh = true;
    for (let j = i - lookback; j < i; j++) {
      if (tenMinContinuous[j].h >= candle.h) {
        isSwingHigh = false;
        break;
      }
    }
    if (isSwingHigh) {
      for (let j = i + 1; j <= i + lookforward; j++) {
        if (tenMinContinuous[j].h >= candle.h) {
          isSwingHigh = false;
          break;
        }
      }
    }
    if (isSwingHigh) {
      swings.push({
        time: candle.t,
        price: candle.h,
        confirmedAtTime: tenMinContinuous[i + lookforward].t,
      });
    }
  }
  return swings;
}

// First candle in `series` (already sorted ascending) whose CLOSE trades
// above `level`, at or after `afterTime`, on qualifying volume — the
// shared "did price break this level" check used for both the 5-min and
// 10-min swing-high break columns.
export function findBreakAfter(series, afterTime, level, { requireVolume, hasVolumeData, rollingAvgByTime, volumeMultiplier }) {
  for (const bar of series) {
    if (bar.t <= afterTime) continue;
    if (bar.c <= level) continue;
    if (!goodVolumeCheck(bar, { requireVolume, hasVolumeData, rollingAvgByTime, volumeMultiplier })) continue;
    return { time: bar.t, price: bar.c, volume: bar.v };
  }
  return null;
}
