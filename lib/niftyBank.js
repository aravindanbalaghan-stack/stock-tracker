...
// dataset (not per-day) — see the routes that call this.
export function detectOpeningRangeBreakout(dayBars, { volumeMultiplier = GOOD_VOLUME_MULTIPLIER, hasVolumeData = true } = {}) {
  ...
  for (let i = 1; i < dayBars.length; i++) {
    ...
    // If there's no real volume data at all, the volume condition can't
    // meaningfully gate anything — treat it as satisfied rather than as
    // an unmeetable bar. When real volume IS present, this is an
    // unchanged relative-volume check.
    const goodVolume = !hasVolumeData || (avgVolSoFar > 0 && (bar.v ?? 0) > avgVolSoFar * volumeMultiplier);
