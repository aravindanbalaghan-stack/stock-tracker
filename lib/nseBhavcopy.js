  ...
  try {
    ...
    for (const r of rows) {
      ...
      bySymbol.set(symbol, {
        symbol,
        series,
        // OHLC: previously only close/prevClose were kept, since the
        // delivery screens never needed the rest. The Chartink-style
        // screeners do (gap-ups compare open vs. previous close, pocket
        // pivots need intraday down-days and the daily low), and the
        // columns are already in the CSV, so parsing them costs nothing.
        open: parseFloat(r.OPEN_PRICE) || null,
        high: parseFloat(r.HIGH_PRICE) || null,
        low: parseFloat(r.LOW_PRICE) || null,
...
/**
 * Fetch the first available bhavcopy at or before `targetDate`, walking
 * backward past weekends/holidays. Used by the IPO Base scan, which needs
 * a single snapshot of "which symbols existed 15 months ago" rather than a
 * continuous history — one file instead of ~300.
 */
export async function getBhavcopyNear(targetDate, maxSearchDays = 12) {
