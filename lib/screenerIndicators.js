...
import { istDateOf } from "@/lib/yahooFinance";
...
export async function fetchDailyOHLCV(symbol, range = "2y") {
  ...
  try {
    ...
    // Bug fix: on non-trading days (weekends, and NSE holidays in
    // particular) Yahoo's chart endpoint sometimes still appends a
    // trailing bar stamped with TODAY's timestamp, carrying forward the
    // last real session's OHLC as if it were a new close. Nothing here
    // was checking for that, so on a holiday the "Levels" section of the
    // stock insight page (52-week high/low, EMA21, WMA30 — all computed
    // from bars[bars.length-1]) would silently show the previous
    // session's numbers as though they were current — exactly the same
    // failure mode app/api/quote/route.js already had to solve for live
    // quotes (see its comment on chartPreviousClose/bars[length-2]).
    //
    // meta.regularMarketTime is Yahoo's own authoritative "as of"
    // timestamp for this instrument, independent of the bars array — so
    // it's used here as ground truth for which calendar day the data
    // actually belongs to. Any trailing bar dated AFTER that real session
    // date is a placeholder, not a second close, and is dropped.
    const meta = result.meta || {};
    const sessionEpoch = meta.regularMarketTime ?? null;
    if (sessionEpoch != null && bars.length > 0) {
      const sessionDate = istDateOf(sessionEpoch);
      while (bars.length > 0 && istDateOf(bars[bars.length - 1].t) > sessionDate) {
        bars = bars.slice(0, -1);
      }
    }

    const lastBarDate = bars.length ? istDateOf(bars[bars.length - 1].t) : null;

    return {
      bars,
      // The real calendar date the last bar in `bars` belongs to, after
      // the placeholder-bar trim above — callers that display price
      // levels derived from the last bar (52w high/low, EMA, WMA) should
      // show this alongside them, rather than implying it's always
      // "today's" price.
      asOf: lastBarDate,
    ...
    console.error(`fetchDailyOHLCV failed for ${symbol}:`, err?.message || err);
    return null;
  }
}
